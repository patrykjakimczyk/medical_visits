...
@Component({
  selector: 'app-edit-patient-for-admin',
  templateUrl: './edit-patient-for-admin.component.html',
  styleUrls: ['./edit-patient-for-admin.component.css', '../../../forms_css.css']
})
export class EditPatientForAdminComponent {
  ...
  assignDoctor(doctor: DoctorListElem) {
    this.patientData.assignedDoctor = doctor;
    this.doctors = null;
  }

  searchForDoctors(input: string) {
    const filterType = this.filterType ? this.filterType : FilterTypeParam.FIRST_NAME;
    ...
    this.apiService.getDoctors(0, 100, filterType, input).subscribe({
      next: (response: any) => {
